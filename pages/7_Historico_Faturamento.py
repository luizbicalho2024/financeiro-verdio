...
if not history: st.info("Sem dados."); st.stop()
...
c = alt.Chart(chart_data).mark_bar().encode(x='periodo_relatorio', y='valor_total', tooltip=['periodo_relatorio', 'valor_total']).properties(height=250)
...
with c1:
    ...
    # Filtra colunas para não mostrar lixo
    df_show = df[['cliente', 'periodo_relatorio', 'valor_total', 'Data', '_id']].rename(columns={'cliente': 'Cliente', 'periodo_relatorio': 'Mês', 'valor_total': 'Total'})
...
with c2:
    ...
    if sel_data:
        ...
        if itens:
            st.dataframe(pd.DataFrame(itens)[['Terminal', 'Valor a Faturar']], hide_index=True, use_container_width=True, height=200)
        else:
            st.caption("Sem itens detalhados salvos.")
            
        if st.button("🗑️ Excluir Registro", type="primary"):
            umdb.delete_billing_history(sel_data['_id'])
            st.rerun()
    else:
