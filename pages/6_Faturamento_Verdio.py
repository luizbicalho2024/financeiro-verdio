...
def create_pdf_report(nome_cliente, periodo, totais, dfs):
    ...
    headers = ["Nº Fat. Cheio", "Nº Proporcional", "Nº Suspensos", "Total GPRS", "Total Satelitais"]
    ...
    def draw_table(title, df, widths):
        ...
        for _, row in df.iterrows():
            for c in cols:
                txt = str(row.get(c, ''))
                if 'Valor' in c and isinstance(row.get(c), (int, float)):
                    txt = f"R$ {row[c]:,.2f}"
                elif isinstance(row.get(c), datetime):
                    txt = row[c].strftime('%d/%m/%Y')
...
for k in sorted(pricing_config.keys()):
    ...
    with cols[i % 4]:
        prices[k] = st.number_input(f"Preço {k}", value=float(val), format="%.2f")
    i += 1
...
if not_found:
    st.warning(f"⚠️ {len(not_found)} Equipamentos não encontrados no estoque.")
    with st.expander("Ver não encontrados"):
        st.json(not_found)
...
log_data = {
    ...
    "terminais_proporcional": len(df_ativ)+len(df_desat), 
    "terminais_suspensos": len(df_susp), 
    "terminais_gprs": len(df_final[df_final['Tipo']=='GPRS']), 
...
st.markdown("---")
with st.expander("Visualizar Tabelas Detalhadas"):
    st.write("Cheio")
    st.dataframe(df_cheio, use_container_width=True)
    st.write("Proporcional")
    st.dataframe(df_ativ, use_container_width=True)
    st.write("Desativados")
    st.dataframe(df_desat, use_container_width=True)
