...
def initialize_firebase():
    ...
    Esta função lê as credenciais do secrets.toml, que deve estar formatado
    corretamente com chaves e valores individuais, não como um único bloco JSON.
    """
    if not firebase_admin._apps:
        try:
            ...
            # Para a execução do app se a inicialização do Firebase falhar.
            st.error(f"Falha ao inicializar o Firebase: {e}")
            st.stop()
    
    # Retorna o cliente do Firestore para ser usado por outras funções.
...
def get_user_role(db, uid):
    ...
    Retorna 'Usuário' como padrão se não encontrar a role ou o documento.
    """
    try:
    ...
    return 'Usuário' # Retorno padrão em caso de erro ou ausência de role.
    ...
    try:
        ...
        for user in users_ref.iterate_all():
            user_data = {
                ...
                "role": get_user_role(db, user.uid) # Busca a role individualmente
            }
...
def create_new_user(db, email, password, role):
    """
    Cria um novo usuário no Firebase Auth e, em seguida, define sua role no Firestore.
    """
    try:
        # 1. Cria o usuário no Firebase Authentication
        new_user = auth.create_user(email=email, password=password)
        
        # 2. Adiciona o nível de acesso (role) e email no Firestore
